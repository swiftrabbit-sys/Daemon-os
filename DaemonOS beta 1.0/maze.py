...
def makeColor(row, col, rows, cols):
    if (((row == rows-1) and (col == cols-2)) or
        ...
        green = int(255*(row+col)/float(rows+cols))
        blue = int(255*(rows+cols-row-col)/float(rows+cols))
        red = 40
        color = hexColor(int(red), int(green), int(blue))
...
class Seg(object):
    ...
    def __str__(self):
        return f"({self.p1.x:.6f},{self.p1.y:.6f})-({self.p2.x:.6f},{self.p2.y:.6f})"
...
class Vector(Matrix):
    def __init__(self, elements):
        # column vector
        self.elements = list(elements) if hasattr(elements, '__iter__') else elements
    ...
    def __mul__(self, other):
        ...
        return Vector(list(map(lambda x : other*x, self.elements)))

    def __rmul__(self, other):
        return self.__mul__(other)

    def __add__(self, other):
        newElements = list(map(sum, zip(self.elements, other.elements)))
        return Vector(newElements)

    def __neg__(self):
        return Vector(list(map(lambda x: -x, self.elements)))
...
class Intersection(object):
    ...
    def __str__(self):
        if (self.kind == "infinity"):
            ...
            return f"({self.point.x:.6f},{self.point.y:.6f})"

    def __repr__(self):
        return f"Intersection({repr(self.point)},{self.kind})"
